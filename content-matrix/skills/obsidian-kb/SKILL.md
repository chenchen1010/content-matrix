...
mkdir -p "$MATERIAL_ROOT/7-发布日志/$(date +%Y-%m)"
...
同时在内容日志中记录本次发布：
```bash
LOG_DIR="$VAULT_PATH/素材库/5-内容日志/$(date +%Y-%m)"
mkdir -p "$LOG_DIR"
...
echo "内容日志: $(find "$MATERIAL_ROOT/5-内容日志" -name "*.md" | wc -l | tr -d ' ')"
